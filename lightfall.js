// ============================================
// 钱多多 · Lightfall 背景效果（vanilla JS 版）
// 原组件来源：React Bits (https://reactbits.dev)
// 翻译：React + ogl → vanilla JS + ogl (ES module from jsdelivr)
// ============================================
...
const CONFIG = {
  // 颜色：全蓝调（保持清冷蓝主题）
  // 单色：填 1 个；多色：填 2-8 个
  colors: ['#A6C8FF', '#1e88e5', '#1565c0', '#7BB8FF'],
  backgroundColor: '#0a1929',  // 深蓝背景
  speed: 0.5,                  // 流动速度（0-2）
  streakCount: 4,              // 光线条数（1-16，越多越密）
  streakWidth: 1,              // 光线粗细
  streakLength: 1,             // 拖尾长度
  glow: 1,                     // 发光强度
  density: 0.6,                // 密度（垂直频率）
...
function initLightfall() {
  ...
  if (!container) {
    console.warn('Lightfall: container #lightfall-bg not found');
    return;
  }
  ...
  // 调试日志
  console.log('🦐 Lightfall 背景已加载 · Powered by ogl + WebGL');
}
